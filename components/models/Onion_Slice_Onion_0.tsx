...
type GLTFResult = GLTF & {
  nodes: {
    Onion_Slice_Onion_0: THREE.Mesh;
  };
  materials: {
...
export function OnionInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Onion_Slice_Onion_0.glb")).uri
  ) as GLTFResult;
  const instances = React.useMemo(
    () => ({
      OnionSliceOnion: nodes.Onion_Slice_Onion_0,
    }),
    [nodes]
  );
  return (
...
export function OnionModel(props: JSX.IntrinsicElements["group"]) {
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.OnionSliceOnion scale={0.01} />
    </group>
  );
}
