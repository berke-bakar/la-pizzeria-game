...
type GLTFResult = GLTF & {
  nodes: {
    Tomato_Slice_Tomato_0: THREE.Mesh;
  };
  materials: {
...
export function TomatoInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Tomato_Slice_Tomato_0.glb"))
      .uri
  ) as GLTFResult;
  const instances = React.useMemo(
    () => ({
      TomatoSliceTomato: nodes.Tomato_Slice_Tomato_0,
    }),
    [nodes]
  );
  return (
...
export function TomatoModel(
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.TomatoSliceTomato scale={0.01} />
    </group>
  );
}

useGLTF.preload(
  Asset.fromModule(require("../../assets/models/Tomato_Slice_Tomato_0.glb")).uri
);
