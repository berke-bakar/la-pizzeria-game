...
export function HamInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Ham_Slice_Ham_0.glb")).uri
  ) as GLTFResult;
  const instances = React.useMemo(
    () => ({
      HamSliceHam: nodes.Ham_Slice_Ham_0,
    }),
    [nodes]
  );
  return (
...
export function HamModel(props: JSX.IntrinsicElements["group"]) {
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.HamSliceHam scale={0.01} />
    </group>
  );
}
