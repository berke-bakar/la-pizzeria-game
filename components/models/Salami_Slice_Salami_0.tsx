...
type GLTFResult = GLTF & {
  nodes: {
    Salami_Slice_Salami_0: THREE.Mesh;
  };
  materials: {
...
export function SalamiInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Salami_Slice_Salami_0.glb"))
      .uri
  ) as GLTFResult;
  const instances = React.useMemo(
    () => ({
      SalamiSliceSalami: nodes.Salami_Slice_Salami_0,
    }),
    [nodes]
  );
  return (
...
export function SalamiModel(props: JSX.IntrinsicElements["group"]) {
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.SalamiSliceSalami scale={0.01} />
    </group>
  );
}

useGLTF.preload(
  Asset.fromModule(require("../../assets/models/Salami_Slice_Salami_0.glb")).uri
);
