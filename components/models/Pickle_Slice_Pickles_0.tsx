...
type GLTFResult = GLTF & {
  nodes: {
    Pickle_Slice_Pickles_0: THREE.Mesh;
  };
  materials: {
...
export function PickleInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Pickle_Slice_Pickles_0.glb"))
      .uri
  ) as GLTFResult;

  const instances = React.useMemo(
    () => ({
      PickleSlicePickles: nodes.Pickle_Slice_Pickles_0,
    }),
    [nodes]
  );
  return (
...
export function PickleModel(props: JSX.IntrinsicElements["group"]) {
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.PickleSlicePickles scale={0.01} />
    </group>
  );
}
