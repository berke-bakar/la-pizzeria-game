...
type GLTFResult = GLTF & {
  nodes: {
    Shrimp_Slice_Shrimp_0: THREE.Mesh;
  };
  materials: {
...
export function ShrimpInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Shrimp_Slice_Shrimp_0.glb"))
      .uri
  ) as GLTFResult;
  const instances = React.useMemo(
    () => ({
      ShrimpSliceShrimp: nodes.Shrimp_Slice_Shrimp_0,
    }),
    [nodes]
  );
  return (
...
export function ShrimpModel(
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.ShrimpSliceShrimp scale={0.01} />
    </group>
  );
}

useGLTF.preload(
  Asset.fromModule(require("../../assets/models/Shrimp_Slice_Shrimp_0.glb")).uri
);
