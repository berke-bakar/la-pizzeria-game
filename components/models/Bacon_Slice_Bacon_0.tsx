...
type GLTFResult = GLTF & {
  nodes: {
    Bacon_Slice_Bacon_0: THREE.Mesh;
  };
  materials: {
...
export function BaconInstances({
  children,
  ...props
}: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    Asset.fromModule(require("../../assets/models/Bacon_Slice_Bacon_0.glb")).uri
  ) as GLTFResult;
  const instances = React.useMemo(
    () => ({
      BaconSliceBacon: nodes.Bacon_Slice_Bacon_0,
    }),
    [nodes]
  );
  return (
...
export function BaconModel(
  ...
  return (
    <group
      ...
      dispose={null}
    >
      <instances.BaconSliceBacon scale={0.01} />
    </group>
  );
}
